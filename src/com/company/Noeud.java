...
public class Noeud { // Un noeud de l'espace de recherche
    Probleme probleme;
    ArrayList<Integer> solution_partielle; // Solution partielle dans le noeud
    ArrayList<Integer> reste_solution; // Le reste des noeud non encore affecté
    ArrayList<HashMap<Integer, Integer[]>> matrice_in_out; // cette matrice contient la date début et fin de chaque tache dans la solution partielle
    int upper_bound;
    ...
    public Noeud(ArrayList<Integer> sol_part, Probleme probleme) {
        ...
        // Remplissage du reste de la solution
        for (int i = 0; i < probleme.nombre_taches; i++) {
            if (!sol_part.contains(i)) reste_solution.add(i);
        }
        // Remplissage de la matrice in-out (Calcule de la date début et fin dans la solution partielle)
        int time_d = 0;
        int time_fin = 0;
